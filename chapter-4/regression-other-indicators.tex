...
Zur Sicherstellung positiver Werte wird die Transformation auf $(1 + \text{water\_use}_{\text{total}})$ angewendet.
Die Zielvariable ist damit
\[
  \text{bc\_wu} =
  \begin{cases}
    \dfrac{(1+\text{water\_use}_{\text{total}})^{\lambda}-1}{\lambda}, & \lambda \neq 0, \\
    \log(1+\text{water\_use}_{\text{total}}), & \lambda = 0 .
  \end{cases}
\]
...
Zur Behandlung von Nullwerten wird die Transformation auf $(1+y)$ angewendet,
wobei $y$ den terrestrischen Eutrophierungs Gesamtwert in $\mathrm{mol\ N}$
bezeichnet.

\[
  y_{\mathrm{BC}} =
  \begin{cases}
    \dfrac{(1+y)^{\lambda}-1}{\lambda}, & \lambda \neq 0, \\
    \log(1+y), & \lambda = 0.
  \end{cases}
\]
