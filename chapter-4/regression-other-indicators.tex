...
Insgesamt zeigen die meisten Indikatoren eine moderate bis hohe Testgüte (\(R^2_{\text{Test}}\approx 0.73\) bis \(0.87\)).
Die Kombination aus RMSE und Median absoluter Fehler verdeutlicht eine typische Schiefe der Fehlerverteilungen.
Der RMSE wird von wenigen sehr großen Abweichungen dominiert, 
während der Median den typischen Fehler eines repräsentativen Produkts beschreibt.

Die relativen Fehlermaße \(\mathrm{MdARE}\) und \(\mathrm{MARE}\) sind einheitenlos und als Anteil interpretierbar, 
zum Beispiel entspricht \(\mathrm{MdARE}=0.66\) einer typischen relativen Abweichung von etwa \(66\,\%\).
Dass \(\mathrm{MARE}\) in allen aufgezeigten Fällen deutlich größer als \(\mathrm{MdARE}\) ist, weist auf eine stark 
rechtsschiefe Verteilung der relativen Fehler hin.
Dies tritt insbesondere bei sehr kleinen Zielwerten auf, 
da dort bereits kleine absolute Abweichungen zu sehr großen relativen Fehlern führen.
...
Die Zielvariable ist damit $\log(1+\text{acidification}_{\text{total}})$. Es konnten $n=177$ PEPs
verwendet werden.
...
\(\mathrm{MdARE}\approx 0{,}96\) bedeutet, dass die typische Abweichung in der Größenordnung des Zielwerts liegt.
...
\begin{figure}[h]
  ...
  logarithmisch skaliert. [Eigene Darstellung]}
...
leicht unterschätzt und im Bereich von $10^1$ ist ein Cluster sichtbar, der tendenziell überschätzt wird..
...
einen QQ Plot der Residuen des Modells im Vergleich zu einer Normalverteilung.
Wie beim CO$_2$ Indikator zeigen sich auf der Originalskala 
typischerweise starke Abweichungen, weshalb die Fehlerdiagnostik hier auf der Transformationsskala berichtet wird.
...
\begin{figure}[h]
  ...
  \caption{QQ Plot der Residuen des Acidification Modells (Test- und Trainingsset). [Eigene Darstellung]}
...
Transformationen der Zielvariable verglichen. Box-Cox verbessert die Testgüte teilweise leicht. 
Insgesamt bleiben die Zugewinne jedoch begrenzt und führen nicht zu stabilen Vorhersagen.
...
($R^2_{\text{Test}}\approx 0{,}49$), und die relativen Fehlermaße
...
\begin{figure}[H]
  ...
  \caption{Vorhergesagte gegenüber tatsächlichen Werten des Indikators
           \emph{Radioactive waste disposed}. [Eigene Darstellung]}
  \label{fig:reg_rwd}
\end{figure}
...
\begin{figure}[h]
  ...
  \caption{QQ Plot der Residuen des Modells für \emph{Radioactive waste disposed}
