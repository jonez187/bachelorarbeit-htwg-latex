...
dargestellt (Abb.~\ref{fig:missingness}). Die Missingness umfasst sowohl Werte,
die die Datenpipeline nicht extrahieren konnte, als auch Werte, die in den PEPs nicht berichtet werden.
...
Indikatoren wie \emph{Eutrophierung mariner Gewässer}, \emph{Klimawandel (fossil,
...
basieren. Konkret wurden PEPs identifiziert, die auf PEF~3.0 und weitere, die auf
...
\begin{figure}[h]
  ...
  \caption{Anteil fehlender Werte pro Umweltindikator (\%, \(N=234\) Produkte). [Eigene Darstellung]}
...
\begin{figure}[h]
  ...
  \caption{Erscheinungsjahre der analysierten PEP-Dokumente (\(N=233\) Produkte). [Eigene Darstellung]}
...
Die Kennzahlen in Tabelle~\ref{tab:basis} beziehen sich jeweils auf die verfügbaren, nicht fehlenden Werte der entsprechenden Variable.
Sie zeigen deutlich
...
\begin{figure}[ht]
  ...
  \caption{Histogramme des Gesamtgewichts auf Originalskala (links, bis zu 1.000.000 g) und nach log1p-Transformation (rechts). [Eigene Darstellung]}
...
\begin{figure}[ht]
  ...
  \caption{Histogramme des Stromverbrauchs auf Originalskala (links, bis zu 1.000.000 kWh) und nach log1p-Transformation (rechts). [Eigene Darstellung]}
  \label{fig:hist_elec}
\end{figure}
...
\begin{figure}[h] 
  ...
  \caption{Verteilung der verwendeten Energiemodelle (ISO-Regionen) über die Lebenszyklusphasen. [Eigene Darstellung]}
  \label{fig:iso_energy}
\end{figure}

\FloatBarrier

Eine weitere Variable, die die Umweltindikatoren stark beeinflussen, und damit in der zu entwickelnden Heuristik eine Rolle spielt,
...
Für die Mittelwerte wurde pro Produkt ein fehlendes Material als Anteil 0~\% behandelt, 
sodass die Kennzahlen die durchschnittliche Zusammensetzung über den gesamten Datensatz abbilden.
...
\begin{figure}[tbp]
  ...
  \caption{Boxplots der Total-Werte je Indikator auf logarithmischer Skala. [Eigene Darstellung]}
