\section{Explorative Modellentwicklung}
\label{sec:exp_models}

Dieses Kapitel beschreibt die Wahl des Regressionsmodells. 
Im Mittelpunkt steht die
explorative Entwicklung und Auswertung verschiedener
Modellvarianten, aus denen das in Abschnitt~\ref{sec:regression_co2}
eingesetzte Hauptmodell für die CO$_2$-Äquivalente und die übrigen
Indikatoren abgeleitet wird.
...
welche Kombinationen sich in der Praxis als robust erweisen und welche
Ansätze verworfen wurden.

Vor der Modellschätzung wurden die kontinuierlichen Eingangsvariablen standardisiert.
Hierfür wurde der \texttt{StandardScaler} aus \emph{scikit-learn} verwendet, der jedes Feature
gemäß $x' = (x - \mu_{\text{train}})/\sigma_{\text{train}}$ transformiert.
Dabei werden Mittelwert $\mu_{\text{train}}$ und Standardabweichung $\sigma_{\text{train}}$
ausschließlich auf dem Trainingsdatensatz geschätzt und anschließend unverändert auf
Validierungs- und Testdaten angewendet (\texttt{with\_mean=True, with\_std=True}).
Im \emph{Basisblock} betrifft dies \texttt{log\_w} und \texttt{log\_e}.
Bei der globalen PCA wird die Standardisierung entsprechend auf alle Regressoren gemeinsam angewendet.


\subsection{Experimentelle Fragestellungen}
...
\begin{enumerate}
  ...
  \item \textbf{Beitrag der Energiemodelle:}
        Verbessert sich die Vorhersagegüte, wenn zusätzlich Energiemodelle 
        einbezogen werden?
...
\subsection{Vergleich der Feature-Sets}
...
Verglichen wurden mehrere Feature-Sets, jeweils in einem linearen
Regressionsmodell auf der Transformationsskala
$\log(1+\text{CO2}_\text{total})$:

\begin{itemize}
  \item \textbf{Basis:} nur Gewicht und Stromverbrauch.
  \item \textbf{Basis + Energiemodelle:} Gewicht und Stromverbrauch + Energiemodelle.
  ...
  \item \textbf{Globale PCA (Basis + Rohmaterialien mit Minimalvorkommen):} Die PCA
        wird auf alle Features global angewendet.
\end{itemize}

Für alle Varianten wurden dieselben Train/Test-Aufteilungen verwendet,
...
\begin{table}[h]
  ...
  \caption{Vergleich verschiedener Feature-Sets für den Indikator
           ...
           $\log(1+\text{CO2}_\text{total})$. RMSE nach Rücktransformation}
  \label{tab:reg_exp_feature_sets}
  \begin{tabular}{lccc}
    \toprule
    Modellvariante                     & $R^2_{\text{Train}}$ & \textbf{$R^2_{\text{Test}}$} & RMSE\textsubscript{Test} \\
    \midrule
    Basis (Gewicht, Strom)             & 0.817 & \textbf{0.770} & $50104.33\,\mathrm{kg\ CO_2}$ \\
    Basis + Energiemodelle             & 0.810 & \textbf{0.501} & $159778.91\,\mathrm{kg\ CO_2}$ \\
    Basis + Rohmaterialien             & 0.904 & \textbf{0.567} & $101227.08\,\mathrm{kg\ CO_2}$ \\
    Basis + Rohmaterialien ($n >=10$)  & 0.842 & \textbf{0.832} & $41910.11\,\mathrm{kg\ CO_2}$ \\
    Basis + PCA-Materialien ($n >=10$) & 0.887 & \textbf{0.882} & $33106.20\,\mathrm{kg\ CO_2}$ \\
    PCA auf alle Variablen             & 0.822 & \textbf{0.738} & $53991.70\,\mathrm{kg\ CO_2}$ \\
    \bottomrule
  \end{tabular}
\end{table}

Das Basismodell aus Gewicht und Stromverbrauch erklärt bereits einen
großen Anteil der Varianz der CO$_2$-Äquivalente.

Obwohl die Einbeziehung des verwendeten Energiemodells intuitiv eine höhere Modellgüte
erwarten lässt, liegt die Güte weit unter der des Basismodells. Ein plausibler Grund ist die 
starke Dominanz weniger Strommixe, insbesondere des europäischen
und französischen Mixes (siehe Abb.~\ref{fig:iso_energy}).
Für die anderen Energiemodelle sind in den betrachteten PEPs zu wenig Datenpunkte
um deren Effekt zuverlässig zu schätzen. 
Ein weiterer Grund ist die unzureichende Abdeckung der Jahreszahlen der benutzten Strommixe in den PEPs.
Sie sind nur in wenigen Datensätzen vorhanden und können daher nicht
robust als erklärende Variable modelliert werden.
Gleichzeitig kann sich die Stromerzeugungsstruktur zwischen Jahren deutlich verändern \cite{fr-energy-mix}.
Wenn unterschiedliche Jahre im Datensatz vermischt werden, entsteht zusätzliche Varianz in den
Zielwerten, die durch das benutzte Feature-Set nicht erklärt werden kann und sich als Rauschen im
Modell zeigt.

Die Er\-wei\-terung um
Roh\-ma\-terial\-an\-tei\-le erhöht zwar das Trainings-$R^2$ deutlich, bringt
...
Modellentwicklung und den späteren Regressionsansatz gewählt.


\subsection{Vergleich der Regressionsverfahren}
...
zeigen, damit deutlich bedeutend als der Einfluss des Feature-Sets.
...
\begin{table}[h]
  ...
  \begin{tabular}{lccc}
    ...
    \midrule
    LinearRegression                   & 0.887 & \textbf{0.882} & $33106.20\,\mathrm{kg\ CO_2}$ \\
    Ridge                              & 0.891 & \textbf{0.896} & $25117.31\,\mathrm{kg\ CO_2}$ \\
    Lasso                              & 0.891 & \textbf{0.896} & $25116.20\,\mathrm{kg\ CO_2}$ \\
    \bottomrule
  \end{tabular}
\end{table}
